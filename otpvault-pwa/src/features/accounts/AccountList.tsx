...
import type { AccountEntry, TotpCode } from '../../types'
...
export function AccountList({ accounts, onAdd, onSettings, onDelete, onLock }: AccountListProps) {
  ...
  const fetchCodes = useCallback(async () => {
    ...
    for (const acc of accounts) {
      try {
        const code: TotpCode = await generateTotp(acc.secret_encrypted, acc.algorithm, acc.digits, acc.step)
        newCodes.set(acc.id, { code: code.code, remaining: code.remaining, total: code.total })
      } catch (e) {
  ...
  return (
    <AppLayout title={t('accounts.title')} onSettings={onSettings}>
                  ...
                  return (
                    <div key={acc.id} className="relative group">
                      ...
                      <button
                        ...
                        className="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full bg-white dark:bg-surface-700 border border-surface-200 dark:border-surface-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:bg-red-50 dark:hover:bg-red-950"
                      >
                        <svg className="w-3 h-3 text-red-400 group-hover:text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="mt-6">
        <Button variant="ghost" fullWidth onClick={onLock}>
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
          </svg>
          {t('vault.lock')}
