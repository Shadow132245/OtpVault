...
export function OnboardingScreen({ onSignUp, onSignIn, onError, defaultTab }: OnboardingScreenProps) {
  ...
  const [tab, setTab] = useState<'signup' | 'signin'>(defaultTab ?? 'signup')
          ...
          {tab === 'signup' && (
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <span className="text-xs text-surface-500 dark:text-surface-400 group-hover:text-surface-700 dark:group-hover:text-surface-300 transition-colors">
                {t('legal.agree').split('<1>')[0]}
                <button type="button" className="text-primary-500 hover:text-primary-600 underline mx-0.5" onClick={() => setLegalView('terms')}>{t('legal.terms')}</button>
                {t('legal.agree').includes('</1>') ? t('legal.agree').split('</1>')[1]?.split('<2>')[0] : ' and '}
                <button type="button" className="text-primary-500 hover:text-primary-600 underline mx-0.5" onClick={() => setLegalView('privacy')}>{t('legal.privacy')}</button>
              </span>
            </label>
          )}
